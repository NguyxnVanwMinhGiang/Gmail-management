...
import { getGoogleUsers, updateGoogleUsersStatus, type GoogleUserItem,  } from "../../api/userApi";
import Alert from "../ui/Alert";
import Switch from "@mui/material/Switch";

interface UserTableRowProps {
        user: GoogleUserItem;
        onToggleActive: (user: GoogleUserItem) => void;
        onToggleVip: (user: GoogleUserItem) => void;
    }
...
export default function GoogleUserPage() {
    const [users, setUsers] = useState<GoogleUserItem[]>([]);
    ...
    const toggleUserStatus = useCallback(
        async (user: GoogleUserItem, field: "is_active" | "vip") => {
            try {
                const payload = { [field]: !user[field] };
                const updated = await updateGoogleUsersStatus(user.id, payload);
    ...
    const loadUsers = useCallback(async () => {
        try {
            setLoading(true);
            const data = await getGoogleUsers();
            setUsers(data);
        } catch (error) {
            console.error(error);
            showAlert("error", "Không thể tải danh sách Google user");
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    const filteredUsers = useMemo(() => {
        ...
        return users.filter((user) =>
            [user.full_name ?? "", user.email, String(user.id), user.google_id ?? ""].some((value) => value.toLowerCase().includes(query)),
        );
    }, [users, search]);
    ...
    return (
        <main className="p-6 lg:p-8 space-y-6">
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    <h1 className="text-2xl font-bold">Quản lý Google user</h1>
                    <p className="text-sm text-gray-500">Danh sách user đăng nhập bằng Google.</p>
                </div>
            ...
            <div className="rounded-xl border bg-white p-4 shadow-sm flex items-center gap-3">
                ...
                <input
                    ...
                    placeholder="Tìm theo tên, email, Google ID hoặc ID"
                    value={search}
            ...
            <div className="rounded-xl border bg-white shadow-sm overflow-hidden">
                <div className="flex items-center justify-between border-b p-4">
                    <div>
                        <h2 className="font-semibold">Danh sách Google user</h2>
                ...
                <table className="w-full text-left text-sm">
                    <thead className="bg-gray-50 text-gray-600">
                        <tr>
                            ...
                            <th className="px-4 py-3">Google ID</th>
