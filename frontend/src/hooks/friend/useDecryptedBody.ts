...
import { getBody } from "../../api/friends";
import { decryptPGPText } from "../../api/openpgp";

export const useDecryptedBody = (currentMailKey: string | null, friendId: number | null) => {
  ...
  useEffect(() => {
    ...
    const fetchAndDecryptBody = async () => {
      ...
      if (!currentMailKey || friendId === null) {
      ...
      try {
        ...
        const rawBodyData = await getBody(emailId, friendId);
